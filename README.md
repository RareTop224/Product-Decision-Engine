...
не содержит явных availability, stable item id/URL и правила пересчёта USD в
RUB. Поэтому KNS пока является кандидатом на dealer/export feed, а не готовым
`PriceProvider`. Проверка следующего дня не считается отдельным M3-срезом.
